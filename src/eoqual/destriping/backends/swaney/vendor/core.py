# Vendored verbatim from https://github.com/LifeCanvas-Technologies/pystripe
# (pystripe/core.py, function block wavedec..filter_streaks) — MIT License,
# Copyright (c) 2018 Chung Lab. See LICENSE in this directory.
#
# Vendored rather than depended-on: pystripe's setup.py hard-pins exact,
# years-old dependency versions (numpy==1.19.5, scipy==1.5.4,
# scikit-image==0.17.2...), which fail to build at all on Python 3.12 and
# conflict with this project's own numpy/scipy/scikit-image versions —
# see THIRD_PARTY_LICENSES.md. `import pystripe` also unconditionally
# pulls in dcimg/tifffile/imageio/tqdm (its CLI/batch-processing I/O),
# none of which `filter_streaks` itself needs. Not modified below beyond
# trimming to the self-contained wavedec..filter_streaks block (excludes
# pystripe's file I/O, CLI, and batch-processing functions).
